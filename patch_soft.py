...
target = """  // --- Balanced Upscale (Soft yet crisp) ---
...
replace = """  // --- Anti-Aliasing / Soft Denoise Filter ---
  // We completely remove sharpening, as sharpening exaggerates MP4 compression blocks.
  // Instead, we apply a gentle 9-tap Gaussian blur to smooth out the pixelation and 
  // macroblocking from the video compression, giving it a soft, cinematic, fluid look.
  vec2 texel = (1.0 / uTextureSize) * 1.5; // Slightly wider sample to blend blocks
  ...
  vec3 smoothed = (cUL + cUR + cDL + cDR) * 0.0625 + (cUp + cDn + cLf + cRt) * 0.125 + color * 0.25;
  
  color = smoothed; // Use the purely smoothed image to hide pixelation
  // -----------------------------------"""
